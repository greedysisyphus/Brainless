...
export const APP_CHANGELOG = [
  {
    ...
    title: '找日子',
    items: [
      '班表新增「找日子」分頁：挑幾位同事、各自設條件（有上班、休假、下午有空、指定班別），列出從今天起最多人對得上的日子；湊不齊的日子也會列出，並劃掉卡住的人。',
      '班表「今天」的日期列：手機與 iPad 上前一天／後一天按鈕不再被擠出畫面，日期文字也不會壓到日曆圖示。',
      '支援班：備註原文跟最後定案的人不同時，會一起標出來，查得回是誰決定的。',
      '航班資料不再自動捲到下一班飛機，想看上面的航班不用再拉回去；現在時間線照樣標著。',
      '導覽列按鈕會跟著視窗縮小，不再超出畫面被切掉。',
    ],
  },
  {
    version: '1.8.0',
    ...
    items: [
      ...
      '上車名單的文字版只寫名字，不再帶店名。',
      '同事可設定特定日期的例外：那幾天不搭車，或那天從別站上車；可一次指定連續幾天，平常的設定不用改來改去。',
      '同事列表拿掉每列的店名徽章（上面已有分店篩選），並改成固定欄位對齊。',
      '修正部分 iPad 無法下載上車名單圖檔：改用 Blob 並在 iOS 開預覽頁，比照咖啡豆盤點表的做法。',
    ],
  },
  {
    version: '1.7.4',
