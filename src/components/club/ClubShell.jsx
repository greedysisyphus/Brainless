...
function ClubNavItem({ path, label, Icon }) {
  return (
    <NavLink to={path} className={({ isActive }) => `group relative z-10 flex min-h-[104px] w-[112px] shrink-0 flex-col items-center justify-center gap-2 rounded-2xl border px-2 text-center transition-all duration-200 hover:z-20 focus-visible:z-20 sm:w-[120px] ${isActive ? 'border-[#171717] bg-[#171717] text-white shadow-[0_12px_22px_rgba(23,23,23,0.18)]' : 'border-black/10 bg-white/55 text-[#4d4d48] hover:-translate-y-0.5 hover:border-[#ec5836] hover:bg-white hover:text-[#171717] hover:shadow-[0_12px_24px_rgba(23,23,23,0.08)]'}`}>
      {({ isActive }) => <><span className={`grid h-10 w-10 place-items-center rounded-xl border ${isActive ? 'border-white/20 bg-white/10 text-[#ff9b84]' : 'border-black/10 bg-[#f7f6f2] text-[#ec5836] group-hover:bg-[#fff1ed]'}`}><Icon className="h-5 w-5" strokeWidth={1.75} /></span><span className="line-clamp-2 text-[12px] font-bold leading-snug">{label}</span></>}
    </NavLink>
  )
}
  ...
  return (
    <div className="club-shell cw-shell-min-h overflow-x-hidden bg-[#f7f6f2] text-[#171717]">
      <header className="sticky top-0 z-[70] border-b border-black/10 bg-[#f7f6f2]/95 backdrop-blur-xl">
        ...
        <nav className="border-t border-black/10 bg-white/35 px-3 py-3 sm:px-5 lg:px-8" aria-label="主要功能"><div className="scrollbar-hide mx-auto -m-1 flex max-w-[1220px] items-stretch justify-start gap-2 overflow-x-auto p-1 sm:justify-center sm:gap-3">{navigationItems.map((item) => <ClubNavItem key={item.path} {...item} />)}</div></nav>
      </header>
