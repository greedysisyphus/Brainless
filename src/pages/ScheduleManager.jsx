...
import ShiftOverlapTable from '../components/schedule/ShiftOverlapTable'
import EmployeeOverlapDetail from '../components/schedule/EmployeeOverlapDetail'
...
// 為了向後相容，保留原有的 SHIFT_CODES（使用舊版本）
const SHIFT_CODES = SHIFT_CODES_OLD
...
// 月份選擇器選項（只顯示有資料的月份）
const getMonthOptions = (availableMonths = []) => {
  const options = []
  ...
  // 如果有可用的月份資料，只顯示有資料的月份
  if (availableMonths.length > 0) {
    // 按時間順序排序（最新的在前面）
    const sortedMonths = availableMonths.sort((a, b) => b.key.localeCompare(a.key))
    
    sortedMonths.forEach(month => {
      let label = month.label
      
      // 檢查是否包含店別信息
      const storeSuffix = month.key.includes('_') ? month.key.split('_')[1] : null
      if (storeSuffix) {
        const storeName = storeSuffix === 'central' ? '中央店' : 'D7店'
        label += ` (${storeName})`
      }
      
      // 標記當月（基於月份部分）
      const monthPart = month.monthKey || month.key.split('_')[0]
      if (monthPart === currentKey) {
        label += ' (當月)'
      }
      ...
      options.push({
        key: month.key,
        label: label,
        year: parseInt((month.monthKey || month.key).split('-')[0]),
        month: parseInt((month.monthKey || month.key).split('-')[1]),
        lastUpdated: month.lastUpdated
      })
    })
  } else {
    // 如果沒有資料，顯示當月作為預設選項
...
// 匯入功能專用的月份選擇器選項（顯示完整月份範圍）
const getImportMonthOptions = () => {
  const options = []
  const currentDate = new Date()
...
function ScheduleManager() {
  ...
  const [employeeTags, setEmployeeTags] = useState({}) // 同事標記：'regular', 'support', 'excluded'
  const [employeeStores, setEmployeeStores] = useState({}) // 同事店別：'central', 'd7'
  ...
  const [selectedStore, setSelectedStore] = useState('all') // 'all', 'central', 'd7'
  ...
  // 月份管理
  const [selectedMonthForImport, setSelectedMonthForImport] = useState('')
  const [selectedStoreForImport, setSelectedStoreForImport] = useState('central') // 'central' 或 'd7'
  const [monthManagementTab, setMonthManagementTab] = useState('import') // 'import' 或 'manage'
  ...
  useEffect(() => {
    if (availableMonths.length > 0) {
      // 優先選擇當月，如果當月沒有資料則選擇最新的月份
      const currentDate = new Date()
      const currentYear = currentDate.getFullYear()
      const currentMonth = currentDate.getMonth() + 1
      const currentKey = `${currentYear}-${currentMonth.toString().padStart(2, '0')}`
      
      const hasCurrentMonth = availableMonths.some(month => month.key === currentKey)
      if (hasCurrentMonth) {
  ...
  const loadScheduleData = async () => {
    try {
      ...
      scheduleDocs.forEach(doc => {
        ...
        // 檢查是否為月份格式 (YYYY-MM) 或店別格式 (YYYY-MM_store)
        if (doc.id.match(/^\d{4}-\d{2}(_(central|d7))?$/)) {
          // 提取月份部分
          const monthPart = doc.id.split('_')[0]
          availableMonthsList.push({
            key: doc.id,
            monthKey: monthPart, // 用於顯示和選擇
            label: `${monthPart.split('-')[0]}年${monthPart.split('-')[1]}月`,
      ...
      setAvailableMonths(availableMonthsList.sort((a, b) => b.key.localeCompare(a.key)))
      ...
      if (allData.currentMonth) {
        setCurrentMonthSchedule(allData.currentMonth)
        newLastUpdated.current = allData.currentMonth._lastUpdated
      }
      
      if (allData.nextMonth) {
        setNextMonthSchedule(allData.nextMonth)
        newLastUpdated.next = allData.nextMonth._lastUpdated
      }
      
      // 設定新月份格式的更新時間
      availableMonthsList.forEach(month => {
        newLastUpdated[month.key] = month.lastUpdated
      })
  ...
  const loadEmployeeData = async () => {
    try {
      ...
      // 載入同事標記資料
      const tagsSnapshot = await getDocs(collection(db, 'employeeTags'))
      const tagsData = {}
      tagsSnapshot.forEach(doc => {
        tagsData[doc.id] = doc.data().tag || 'regular'
      })
      setEmployeeTags(tagsData)
      
      // 載入同事店別資料
      const storesSnapshot = await getDocs(collection(db, 'employeeStores'))
      const storesData = {}
      storesSnapshot.forEach(doc => {
        storesData[doc.id] = doc.data().store || 'central'
      })
      setEmployeeStores(storesData)
    } catch (error) {
  ...
  const saveEmployeeData = async () => {
    try {
      ...
      // 儲存同事標記資料
      for (const [id, tag] of Object.entries(employeeTags)) {
        await setDoc(doc(db, 'employeeTags', id), { tag })
      }
      
      // 儲存同事店別資料
      for (const [id, store] of Object.entries(employeeStores)) {
        await setDoc(doc(db, 'employeeStores', id), { store })
      }
  ...
  const addEmployee = () => {
    const { id, name, pickupLocation, tag, store } = newEmployee
    if (!id || !name) {
      alert('請填寫職員編號和姓名')
      return
    }
    ...
    setEmployeeTags(prev => ({ ...prev, [id]: tag }))
    setEmployeeStores(prev => ({ ...prev, [id]: store }))
    setNewEmployee({ id: '', name: '', pickupLocation: PICKUP_LOCATIONS[0], tag: 'regular', store: 'central' })
  }
  ...
  const deleteEmployee = (id) => {
    if (confirm(`確定要刪除 ${names[id]} 嗎？`)) {
      ...
      setEmployeeTags(prev => {
        const newTags = { ...prev }
        delete newTags[id]
        return newTags
      })
      setEmployeeStores(prev => {
  ...
  const updateEmployeeTag = (id, newTag) => {
    setEmployeeTags(prev => ({ ...prev, [id]: newTag }))
  }
  
  const updateEmployeeStore = (id, newStore) => {
    setEmployeeStores(prev => ({ ...prev, [id]: newStore }))
  }
        ...
        const employeeName = columns[1].trim()
         ...
         if (employeeId && employeeName && employeeId.match(/^A\d+$/)) {
           ...
           if (columns[2] && columns[2].trim() === '班別排班') {
             ...
             // 處理班別代碼，根據選定月份使用對應的代碼映射
             const processedSchedule = scheduleData.map(shift => {
               const trimmedShift = shift.trim()
               if (!trimmedShift) return '' // 保持空白
               
               // 根據選定的月份獲取對應的班別代碼映射
               const shiftCodes = getShiftCodes(selectedMonthForImport)
               
               // 檢查是否為已知的班別代碼
               if (shiftCodes[trimmedShift]) {
                 return shiftCodes[trimmedShift] // 只顯示中文說明
               }
  ...
  const uploadSchedule = async () => {
    ...
    if (!selectedMonthForImport) {
      alert('請選擇要儲存的月份')
      return
    }
    
    if (!selectedStoreForImport) {
      alert('請選擇要儲存的店別')
      return
    }
    ...
    try {
      ...
      // 根據店別生成不同的文檔 ID
      const storePrefix = selectedStoreForImport === 'central' ? 'central' : 'd7'
      const documentId = `${selectedMonthForImport}_${storePrefix}`
      
      await setDoc(doc(db, 'schedule', documentId), scheduleData)
      console.log(`班表上傳成功到 ${storePrefix} 店的 ${selectedMonthForImport}`)
      ...
      const storeName = selectedStoreForImport === 'central' ? '中央店' : 'D7店'
      alert(`班表已成功儲存到 ${storeName} 的 ${selectedMonthForImport}！`)
      setImportData('')
      setParsedData(null)
      setSelectedMonthForImport('')
      setSelectedStoreForImport('central')
    } catch (error) {
  ...
  const getFilteredEmployees = () => {
    ...
    if (selectedMonth === 'current') {
      ...
      schedule = nextMonthSchedule
    } else {
      // 使用多月份資料
      schedule = allSchedules[selectedMonth] || {}
    }
    ...
    if (selectedStore !== 'all') {
      filteredEmployees = filteredEmployees.filter(employeeId => {
        const store = employeeStores[employeeId] || 'central'
  ...
  const getCurrentSchedule = () => {
    if (selectedMonth === 'current') {
      ...
      // 使用多月份資料
      return allSchedules[selectedMonth] || {}
    }
  }

  // 計算跨月份的平均值統計
  const getCrossMonthStats = (employeeId) => {
    ...
    const stats = {
      ...
      averageEarlyPerMonth: 0,
      averageMidPerMonth: 0,
      averageNightPerMonth: 0,
      averageRestPerMonth: 0,
      averageSpecialPerMonth: 0
    }
    
    // 遍歷所有月份資料
    Object.keys(allSchedules).forEach(monthKey => {
      if (monthKey === '_lastUpdated') return
      
      const monthSchedule = allSchedules[monthKey]
      const employeeSchedule = monthSchedule[employeeId]
    ...
    if (stats.totalMonths > 0) {
      stats.averageShiftsPerMonth = Math.round(stats.totalShifts / stats.totalMonths)
      stats.averageEarlyPerMonth = Math.round(stats.earlyShifts / stats.totalMonths)
      stats.averageMidPerMonth = Math.round(stats.midShifts / stats.totalMonths)
      stats.averageNightPerMonth = Math.round(stats.nightShifts / stats.totalMonths)
      stats.averageRestPerMonth = Math.round(stats.restDays / stats.totalMonths)
      stats.averageSpecialPerMonth = Math.round(stats.specialShifts / stats.totalMonths)
    }
    ...
    <div className="container-custom py-8">
      ...
      {activeTab === 'display' && (
        <div className="space-y-6">
           ...
           {/* 店別選擇器 */}
           <div className="bg-surface/30 rounded-xl p-4 mb-4 border border-white/10">
             <div className="flex items-center justify-between">
               <h3 className="text-lg font-semibold text-white">店別選擇</h3>
             ...
             {viewMode === 'date' ? (
               <DateView 
                 ...
                 onScheduleUpdate={(updatedSchedule) => {
                   // 更新本地狀態
                   if (selectedMonth === 'current') {
                     setCurrentMonthSchedule(updatedSchedule)
                   } else if (selectedMonth === 'next') {
                     setNextMonthSchedule(updatedSchedule)
                   } else {
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-surface/60 to-surface/40 rounded-2xl p-6 border border-white/20 shadow-xl backdrop-blur-sm">
                ...
                <PersonalDashboard 
                  ...
                  crossMonthStats={getCrossMonthStats(selectedStatsEmployee)}
                  selectedMonth={selectedMonth}
                />
              </div>
            </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-6">
          {/* 月份管理分頁切換 */}
          <div className="flex justify-center mb-6">
            <div className="bg-surface/40 rounded-xl p-1 border border-white/10 w-full max-w-2xl">
              <div className="grid grid-cols-2 gap-1">
                <button
                  onClick={() => setMonthManagementTab('import')}
                  className={`px-6 py-3 rounded-lg transition-all text-sm md:text-base ${
                    monthManagementTab === 'import'
                      ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-green-400 border border-green-400/30'
                      : 'text-text-secondary hover:text-white hover:bg-white/5'
                  }`}
                >
                  <DocumentArrowUpIcon className="w-4 h-4 md:w-5 md:h-5 inline mr-2" />
                  匯入班表
                </button>
                <button
                  onClick={() => setMonthManagementTab('manage')}
                  className={`px-6 py-3 rounded-lg transition-all text-sm md:text-base ${
                    monthManagementTab === 'manage'
                      ? 'bg-gradient-to-r from-blue-500/20 to-cyan-500/20 text-blue-400 border border-blue-400/30'
                      : 'text-text-secondary hover:text-white hover:bg-white/5'
                  }`}
                >
                  <CalendarIcon className="w-4 h-4 md:w-5 md:h-5 inline mr-2" />
                  月份管理
                </button>
              </div>
            </div>
          ...
          {monthManagementTab === 'import' && (
            <div className="bg-surface/60 rounded-xl p-6 border border-white/20 shadow-xl backdrop-blur-sm">
             ...
             <div className="mb-6 p-4 bg-surface/40 rounded-lg border border-white/10">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                 {Object.entries(selectedMonthForImport ? getShiftCodes(selectedMonthForImport) : SHIFT_CODES).map(([code, description]) => (
               ...
               {selectedMonthForImport && (
                 <div className="mt-3 p-3 bg-blue-500/20 rounded border border-blue-400/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">目標店別</label>
                <select
                  value={selectedStoreForImport}
                  onChange={(e) => setSelectedStoreForImport(e.target.value)}
                  className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg focus:border-green-400/50 focus:bg-white/10"
                >
                  <option value="central">中央店</option>
                  <option value="d7">D7店</option>
                </select>
              </div>
            </div>
            ...
            {monthManagementTab === 'manage' && (
            <div className="bg-surface/60 rounded-xl p-6 border border-white/20 shadow-xl backdrop-blur-sm">
              <h3 className="text-xl font-bold mb-4 text-blue-300">月份管理</h3>
              
              {/* 可用月份列表 */}
              <div className="mb-6">
                <h4 className="text-lg font-semibold mb-4 text-cyan-300">已儲存的月份</h4>
                {availableMonths.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {availableMonths.map((month) => (
                      <div key={month.key} className="bg-surface/40 rounded-lg p-4 border border-white/10 hover:border-white/20 transition-all">
                        <div className="flex items-center justify-between mb-2">
                          <h5 className="text-white font-semibold">{month.label}</h5>
                          <span className="text-xs text-gray-400">
                            {month.lastUpdated ? new Date(month.lastUpdated).toLocaleDateString() : '未知'}
                          </span>
                        </div>
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-3 py-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 text-blue-300 rounded text-sm transition-all"
                          >
                            查看
                          </button>
                          <button
                            ...
                            className="px-3 py-1 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-400/30 text-purple-300 rounded text-sm transition-all"
                          >
                            分析
                          </button>
                        </div>
              ...
              <div className="bg-surface/40 rounded-lg p-4 border border-white/10">
                <h4 className="text-lg font-semibold mb-4 text-orange-300">快速操作</h4>
          ...
          <div className="bg-surface/40 rounded-xl p-6 border border-white/10">
            ...
            <div className="bg-surface/20 rounded-lg p-4 mb-6 border border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-2">同事類型</label>
                  <select
                    value={newEmployee.tag}
                    onChange={(e) => setNewEmployee(prev => ({ ...prev, tag: e.target.value }))}
                    className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg focus:border-orange-400/50 focus:bg-white/10"
                  >
                    <option value="regular">一般同事</option>
                    <option value="support">支援同事</option>
                    <option value="excluded">排除統計</option>
                  </select>
                </div>
                
                <div>
                  <label className="block text-sm font-semibold mb-2">店別</label>
                  <select
                    value={newEmployee.store}
                    onChange={(e) => setNewEmployee(prev => ({ ...prev, store: e.target.value }))}
                    className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg focus:border-orange-400/50 focus:bg-white/10"
                  >
                    <option value="central">中央店</option>
                    <option value="d7">D7店</option>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    ...
                    <th className="text-left p-2 text-yellow-400">類型</th>
                    <th className="text-left p-2 text-cyan-400">店別</th>
                   ...
                   {Object.keys(names)
                     ...
                     .map(id => (
                      <tr key={id} className="border-b border-white/5">
                        ...
                        <td className="p-2">
                          <select
                            value={employeeTags[id] || 'regular'}
                            onChange={(e) => updateEmployeeTag(id, e.target.value)}
                            className="w-full px-2 py-1 bg-white/5 border border-white/10 rounded focus:border-yellow-400/50 focus:bg-white/10"
                          >
                            <option value="regular">一般同事</option>
                            <option value="support">支援同事</option>
                            <option value="excluded">排除統計</option>
                        ...
                        <td className="p-2">
                          <select
                            value={employeeStores[id] || 'central'}
                            onChange={(e) => updateEmployeeStore(id, e.target.value)}
                            className="w-full px-2 py-1 bg-white/5 border border-white/10 rounded focus:border-cyan-400/50 focus:bg-white/10"
                          >
                            <option value="central">中央店</option>
                            <option value="d7">D7店</option>
                          </select>
...
// 日期視圖組件
function DateView({ schedule, names, displayDates, filteredEmployees, selectedShifts, selectedMonth, onScheduleUpdate, employeeTags = {} }) {
  // 編輯狀態管理
  ...
  // 處理支援同事的休假顯示邏輯
  const getDisplayShift = (employeeId, shift) => {
    if (!shift) return shift
    
    const tag = employeeTags[employeeId] || 'regular'
    // 如果是支援同事且班別是「休」，則顯示為空白
    if (tag === 'support' && shift === '休') {
      return ''
    }
  ...
  const getShiftColor = (shift) => {
    switch (shift) {
      ...
      case '午': return 'bg-green-500 text-white'
      case '晚': return 'bg-blue-500 text-white'
      case '休': return 'bg-gray-500 text-white'
      case '特': return 'bg-orange-500 text-white'
      case 'D7': return 'bg-purple-500 text-white'
      case '高鐵': return 'bg-indigo-500 text-white'
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-white">
          班表管理
          {isEditing && <span className="text-yellow-400 text-sm ml-2">(編輯模式)</span>}
        </h3>
                ...
                {filteredEmployees.map((employeeId, index) => (
                  <tr key={employeeId} className="border-b border-white/10 hover:bg-white/5 transition-all duration-200 group" data-employee-id={employeeId} data-row-index={index}>
                    ...
                    {displayDates.map((date) => {
                      ...
                      const originalShift = currentSchedule[employeeId]?.[date]
                      const shift = getDisplayShift(employeeId, originalShift) // 使用顯示邏輯處理支援同事的休假
                      const isToday = date === new Date().getDate()
                      
                      // 班次篩選邏輯
                      if (selectedShifts && selectedShifts.length > 0) {
                        if (!originalShift || !selectedShifts.includes(originalShift)) {
                          return (
                          ...
                          {isEditing ? (
                            <select
                              value={shift || ''}
                              onChange={(e) => updateShift(employeeId, date, e.target.value)}
                              className={`w-full h-6 md:h-8 px-1 py-0.5 rounded-lg text-center text-xs font-bold bg-white/10 text-white focus:bg-white/20 focus:outline-none focus:ring-2 focus:ring-blue-400/50 ${getShiftColor(shift)}`}
                            >
...
// 同事視圖組件
function EmployeeView({ schedule, names, displayDates, filteredEmployees, selectedShifts, selectedMonth, onScheduleUpdate, employeeTags = {} }) {
  // 編輯狀態管理
  const [isEditing, setIsEditing] = useState(false)
  const [editingSchedule, setEditingSchedule] = useState({})
  const [isSaving, setIsSaving] = useState(false)
                ...
                <div key={date} className="text-center">
                  ...
                  {isEditing ? (
                    <select
                      value={shift || ''}
                      onChange={(e) => updateShift(employeeId, date, e.target.value)}
                      className={`w-full h-8 px-2 py-1 rounded-lg text-center text-xs font-bold bg-white/10 text-white focus:bg-white/20 focus:outline-none focus:ring-2 focus:ring-blue-400/50 ${getShiftColor(shift)}`}
                    >
                      {SHIFT_OPTIONS.map(option => (
                        <option key={option.value} value={option.value} className="bg-gray-800 text-white">
                          {option.label}
...
// 舊的 ShiftOverlapTable 組件已移至 src/components/schedule/ShiftOverlapTable.jsx

// 舊的 EmployeeOverlapDetail 組件已移至 src/components/schedule/EmployeeOverlapDetail.jsx
...
// 班次分配偏差度分析組件
function ShiftBiasAnalysis({ schedule, names, selectedMonth }) {
  // 根據 selectedMonth 計算對應的月份天數和是否包含午班
  let daysInMonth, targetYear, targetMonth, includeAfternoon
  ...
  // 判斷是否包含午班分析（2025年10月及之後）
  includeAfternoon = targetYear > 2025 || (targetYear === 2025 && targetMonth >= 10)
  
  const calculateBiasAnalysis = () => {
    const biasData = {}
    
    // 計算總班次數
    let totalShifts = { early: 0, middle: 0, afternoon: 0, night: 0, rest: 0, special: 0 }
    Object.keys(schedule).forEach(employeeId => {
      if (employeeId === '_lastUpdated') return
    ...
    const idealPerEmployee = {
      ...
      afternoon: totalShifts.afternoon / totalEmployees,
    ...
    Object.keys(schedule).forEach(employeeId => {
      ...
      const actual = { early: 0, middle: 0, afternoon: 0, night: 0, rest: 0, special: 0 }
      ...
      const bias = {
        name: employeeName,
        earlyBias: idealPerEmployee.early > 0 ? ((actual.early - idealPerEmployee.early) / idealPerEmployee.early * 100).toFixed(1) : '0.0',
        middleBias: idealPerEmployee.middle > 0 ? ((actual.middle - idealPerEmployee.middle) / idealPerEmployee.middle * 100).toFixed(1) : '0.0',
        afternoonBias: idealPerEmployee.afternoon > 0 ? ((actual.afternoon - idealPerEmployee.afternoon) / idealPerEmployee.afternoon * 100).toFixed(1) : '0.0',
        nightBias: idealPerEmployee.night > 0 ? ((actual.night - idealPerEmployee.night) / idealPerEmployee.night * 100).toFixed(1) : '0.0',
        restBias: idealPerEmployee.rest > 0 ? ((actual.rest - idealPerEmployee.rest) / idealPerEmployee.rest * 100).toFixed(1) : '0.0',
        specialBias: idealPerEmployee.special > 0 ? ((actual.special - idealPerEmployee.special) / idealPerEmployee.special * 100).toFixed(1) : '0.0'
      }
      
      biasData[employeeName] = bias
    })
    
    return Object.values(biasData).sort((a, b) => {
      // 按職員編號排序
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/20">
              ...
              {includeAfternoon && <th className="text-center p-3 text-orange-300 font-semibold">午班偏差</th>}
              ...
              <th className="text-center p-3 text-orange-300 font-semibold">特休偏差</th>
            </tr>
          </thead>
          <tbody>
            {biasData.map((item, index) => (
              <tr key={index} className="border-b border-white/10 hover:bg-white/5">
                ...
                {includeAfternoon && (
                  <td className={`p-3 text-center font-bold ${parseFloat(item.afternoonBias) > 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {item.afternoonBias}%
                  </td>
                )}
...
function PersonalDashboard({ schedule, names, selectedEmployee, crossMonthStats, selectedMonth }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="text-center p-4 bg-orange-500/20 rounded-lg border border-orange-400/30">
          <div className="text-2xl font-bold text-orange-300">{personalStats.special}</div>
                          <div className="text-sm text-orange-200">特休</div>
        </div>
      </div>
      ...
      {/* 跨月份平均值統計 */}
      {crossMonthStats && crossMonthStats.totalMonths > 0 && (
        <div className="bg-gradient-to-br from-surface/60 to-surface/40 rounded-2xl p-6 border border-white/20 shadow-xl backdrop-blur-sm">
          <h4 className="text-lg font-bold mb-4 text-green-300 text-center">跨月份平均值統計</h4>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-4">
            <div className="text-center p-3 bg-blue-500/20 rounded-lg border border-blue-400/30">
              <div className="text-lg font-bold text-blue-300">{crossMonthStats.totalMonths}</div>
              <div className="text-xs text-blue-200">統計月份</div>
            </div>
            {(() => {
              const currentMonthStats = {
                early: personalStats.early,
                middle: personalStats.middle,
                night: personalStats.night,
                rest: personalStats.rest
              }
              
              const getDifference = (current, average) => {
              ...
              const earlyDiff = getDifference(currentMonthStats.early, crossMonthStats.averageEarlyPerMonth)
              const midDiff = getDifference(currentMonthStats.middle, crossMonthStats.averageMidPerMonth)
              const nightDiff = getDifference(currentMonthStats.night, crossMonthStats.averageNightPerMonth)
              const restDiff = getDifference(currentMonthStats.rest, crossMonthStats.averageRestPerMonth)
              
              return (
                <>
                  <div className="text-center p-3 bg-pink-500/20 rounded-lg border border-pink-400/30">
                    <div className="text-lg font-bold text-pink-300">{crossMonthStats.averageEarlyPerMonth}</div>
                    <div className="text-xs text-pink-200">平均早班/月</div>
                    <div className={`text-xs mt-1 ${earlyDiff.isPositive ? 'text-green-400' : earlyDiff.isZero ? 'text-gray-400' : 'text-red-400'}`}>
                      {earlyDiff.isPositive ? '+' : ''}{earlyDiff.value} vs 本月
                    </div>
                  </div>
                  
                  <div className="text-center p-3 bg-cyan-500/20 rounded-lg border border-cyan-400/30">
                    <div className="text-lg font-bold text-cyan-300">{crossMonthStats.averageMidPerMonth}</div>
                    <div className="text-xs text-cyan-200">平均中班/月</div>
                    <div className={`text-xs mt-1 ${midDiff.isPositive ? 'text-green-400' : midDiff.isZero ? 'text-gray-400' : 'text-red-400'}`}>
                      {midDiff.isPositive ? '+' : ''}{midDiff.value} vs 本月
                    </div>
                  </div>
                  
                  <div className="text-center p-3 bg-blue-500/20 rounded-lg border border-blue-400/30">
                    <div className="text-lg font-bold text-blue-300">{crossMonthStats.averageNightPerMonth}</div>
                    <div className="text-xs text-blue-200">平均晚班/月</div>
                    <div className={`text-xs mt-1 ${nightDiff.isPositive ? 'text-green-400' : nightDiff.isZero ? 'text-gray-400' : 'text-red-400'}`}>
                      {nightDiff.isPositive ? '+' : ''}{nightDiff.value} vs 本月
                    </div>
                  </div>
                  
                  <div className="text-center p-3 bg-gray-500/20 rounded-lg border border-gray-400/30">
                    <div className="text-lg font-bold text-gray-300">{crossMonthStats.averageRestPerMonth}</div>
                    <div className="text-xs text-gray-200">平均休假/月</div>
                    <div className={`text-xs mt-1 ${restDiff.isPositive ? 'text-green-400' : restDiff.isZero ? 'text-gray-400' : 'text-red-400'}`}>
                      {restDiff.isPositive ? '+' : ''}{restDiff.value} vs 本月
                    </div>
                  </div>
                </>
              )
          ...
          <div className="text-center text-sm text-gray-400">
            基於 {crossMonthStats.totalMonths} 個月份的資料計算 | <span className="text-green-400">綠色</span> = 高於平均值 | <span className="text-red-400">紅色</span> = 低於平均值
          </div>
        </div>
      )}
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-3 text-xs">
            ...
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-orange-500 rounded"></div>
                              <span className="text-gray-300">特休</span>
            </div>
          </div>
        </div>
      </div>
...
// 每日搭班視圖組件
function DailyPartnerView({ schedule, names, displayDates, selectedEmployee, selectedMonth }) {
  // 判斷是否使用新的搭班算法（2025年10月及之後）
  const useNewShiftOverlapAlgorithm = (selectedMonth) => {
    if (!selectedMonth) {
      // 如果沒有指定月份，使用當前月份判斷
  ...
  const getPartnersForDay = (day) => {
    ...
    // 如果選中的員工是D7班或高鐵班，不與其他同事搭班
    if (selectedShift === 'D7' || selectedShift === '高鐵') return []
    ...
    Object.keys(schedule).forEach(employeeId => {
      ...
      // 如果同事是D7班或高鐵班，不與其他同事搭班
      if (partnerShift === 'D7' || partnerShift === '高鐵') return
      ...
      // 跨班搭班
        ...
        if (useNewAlgorithm) {
          ...
          // 4. 晚班 會跟 午班 搭班
          if (
            (selectedShift === '早' && (partnerShift === '中' || partnerShift === '午')) ||
            (selectedShift === '中' && (partnerShift === '早' || partnerShift === '午')) ||
            (selectedShift === '午' && (partnerShift === '早' || partnerShift === '中' || partnerShift === '晚')) ||
            (selectedShift === '晚' && partnerShift === '午')
          ) {
            partners.push({
              id: employeeId,
              name: names[employeeId] || employeeId,
      ...
      // 按照班次順序排序：早班 -> 中班 -> 午班 -> 晚班
      const shiftOrder = { '早': 1, '中': 2, '午': 3, '晚': 4 }
      partners.sort((a, b) => {
